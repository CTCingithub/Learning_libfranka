...
MotionGenerator::MotionGenerator(const std::array<double, 7> q_goal,
                                 double speed_factor)
    : q_goal_(q_goal.data()) {
...
bool MotionGenerator::calculateDesiredValues(double t,
  ...
  return std::all_of(joint_motion_finished.cbegin(),
...
MotionGenerator::operator()(const franka::RobotState &robot_state,
